...
//  }
  ...
  if (grid.dumpControl.doRestart) {
    ...
    jsonParser::ariannaFlags flags;
    jsonParser::setAriannaFlags(root, flags);
  ...
  while (grid.istep <= grid.getTotalNumberOfTimesteps())
  {
    ...
    for (spec_iterator = species.begin(); spec_iterator != species.end(); spec_iterator++) {
        ...
        if( grid.getDimensionality()==3 || (!(flags.current_deposition_standard_omp_atom) && !(flags.current_deposition_standard_omp_jaux) && !(flags.current_deposition_esirkepov)) ){
            (*spec_iterator)->current_deposition_standard(&current);
        }

        if(flags.current_deposition_standard_omp_atom){
            if(flags.particle_reordering && !(grid.istep % 100) ) {
                (*spec_iterator)->particle_reordering(&grid);
            }
            (*spec_iterator)->current_deposition_standard_omp_atom(&current);
        }

        if(flags.current_deposition_standard_omp_jaux){
            (*spec_iterator)->current_deposition_standard_omp_jaux(&current);
        }

        if(flags.current_deposition_esirkepov){
    ...
    for (spec_iterator = species.begin(); spec_iterator != species.end(); spec_iterator++) {
      if (grid.isRadiationFrictionEnabled()) {
        (*spec_iterator)->momenta_advance_with_friction(&myfield, grid.getLambda0());
          ...
          if(flags.momenta_advance_omp){
            (*spec_iterator)->momenta_advance_omp(&myfield);
          }
          else{
