...
void SPECIE::momenta_advance_omp(EM_FIELD *ebfield)
{
  ...
  case 3:
    ...
    case 2:
#pragma omp parallel for schedule(auto) private(p,c,gamma_i,hii,wii,hiw,wiw,rr,rh,rr2,rh2,dvol,xx,E,B,u_plus,u_minus,u_prime,tee,ess,dummy,i,i1,j1,k1,i2,j2,k2)
    for (p = 0; p < Np; p++)
    {
      //gamma_i=1./sqrt(1+u0(p)*u0(p)+u1(p)*u1(p)+u2(p)*u2(p));
...
int SPECIE::particle_partition(double pivotValue, int left, int right){
  int index = left;
  for(int i=left; i<right; i++){
    if( pData[pIndex(0,i,Ncomp,Np)]< pivotValue){
      for(int c=0; c<Ncomp; c++){
        double tmp = pData[pIndex(c,index,Ncomp,Np)];
        pData[pIndex(c,index,Ncomp,Np)] = pData[pIndex(c,i,Ncomp,Np)];
        pData[pIndex(c,i,Ncomp,Np)] = tmp;
      }
      index++;
    }
  }
...
void SPECIE::particle_reordering(GRID * grid){
    // partial reordering along x coordinate
    int left = 0;
    int right = Np-1;
    const int Nt = omp_get_max_threads();
    for(int t=1; t<Nt; t++){
      double pivotValue =  grid->rminloc[0] + (t*grid->Nloc[0]*1.0/Nt)*grid->dr[0];
      left = particle_partition(pivotValue,left,right);
    }
}
...
void SPECIE::current_deposition_standard_omp_atom(CURRENT *current)
{
  if (mygrid->withParticles == NO||isFrozen)
    return;
  if (mygrid->isStretched()) {
